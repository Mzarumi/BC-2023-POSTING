...
Page 52187116 "CRM Coupling Item"
{
    ...
    layout
    {
        area(content)
        {
            ...
            group("Coupled Records")
            {
                ...
                fixed(Control10)
                {
                    group("Dynamics NAV Item")
                    {
                        Caption = 'Dynamics NAV Item';
                        //The GridLayout property is only supported on controls of type Grid
                        //GridLayout = Columns;
                        field("No.2"; Rec."No.")
                        {
                    ...
                    group("Dynamics CRM Product")
                    {
                        ...
                        field("CRMProduct.StockWeight"; CRMProduct.StockWeight)
                        {
                            ApplicationArea = Basic;
                        }
                        field("CRMProduct.VendorName"; CRMProduct.VendorName)
                        {
                            ApplicationArea = Basic;
                        }
