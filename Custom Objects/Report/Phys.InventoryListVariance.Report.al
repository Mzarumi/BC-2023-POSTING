...
Report 52185608 "Phys. Inventory List Variance"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Phys. Inventory List Variance.rdlc';
    ...
    dataset
    {
        dataitem(PageLoop;"Integer")
        {
            ...
            dataitem("Item Journal Batch";"Item Journal Batch")
            {
                ...
                column(TemplateName_ItemJnlBatch;"Item Journal Batch"."Journal Template Name")
                {
                }
                column(Name_ItemJournalBatch;"Item Journal Batch".Name)
                {
                }
                dataitem("Item Journal Line";"Item Journal Line")
                {
                    ...
                    column(PostingDt_ItemJournalLine;Format("Item Journal Line"."Posting Date"))
                    {
                    }
                    column(DocNo_ItemJournalLine;"Item Journal Line"."Document No.")
                    {
                        IncludeCaption = true;
                    }
                    ...
                    column(Desc_ItemJournalLine;"Item Journal Line".Description)
                    {
                        IncludeCaption = true;
                    }
                    column(ShotcutDim1Code_ItemJnlLin;"Item Journal Line"."Shortcut Dimension 1 Code")
                    {
                    }
                    column(ShotcutDim2Code_ItemJnlLin;"Item Journal Line"."Shortcut Dimension 2 Code")
                    {
                    }
                    column(LocCode_ItemJournalLine;"Item Journal Line"."Location Code")
                    {
                        IncludeCaption = true;
                    }
                    column(QtyCalculated_ItemJnlLin;"Item Journal Line"."Qty. (Calculated)")
                    {
                        IncludeCaption = true;
                    }
                    column(QtyPhysInventory_ItemJnlLin;"Item Journal Line"."Qty. (Phys. Inventory)")
                    {
                    }
                    ...
                    dataitem(ItemTrackingSpecification;"Integer")
                    {
                        ...
                        column(ReportForNavId_7803; 7803)
                        {
                        }
                        ...
                        begin
                            if ItemTrackingSpecification.Number = 1 then
                        ...
                        begin
                            ...
                            if ReservEntryBuffer.IsEmpty then
                              CurrReport.Break;
                            ItemTrackingSpecification.SetRange(ItemTrackingSpecification.Number,1,ReservEntryBuffer.Count);
                    ...
                    begin
                        if ShowLotSN then begin
                          ...
                          if "Item Journal Line"."Bin Code" <> '' then
                            if not WhseTrckg("Item Journal Line"."Item No.") then begin
                              Note := NoteTxt;
                              ShowSummary := true;
                            end;
                        ...
                        Variance := 0;
                        Variance := "Item Journal Line"."Qty. (Phys. Inventory)" - "Item Journal Line"."Qty. (Calculated)";
                    end;
                }
    ...
    procedure CreateSNLotEntries(var ItemJnlLine: Record "Item Journal Line")
    begin
        EntryNo := 0;
    ...
    procedure CreateReservEntry(ItemJournalLine: Record "Item Journal Line";Qty: Decimal;SerialNo: Code[20];LotNo: Code[20];ItemTracking: Option "None","Lot No.","Lot and Serial No.","Serial No.")
    var
        FoundRec: Boolean;
    begin
