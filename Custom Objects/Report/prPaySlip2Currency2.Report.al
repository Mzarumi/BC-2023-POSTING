...
Report 52185558 prPaySlip2Currency2
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/prPaySlip2Currency2.rdlc';
    Caption = 'PaySlip';
            ...
            dataitem("PR Salary Card"; "PR Salary Card")
            {
                ...
                begin
                    ...
                    *****************************************************************************************************/
                    ...
                    FnUpdatePeriodCurrency("Employee Code", SelectedPeriod);
                        ...
                        repeat
                            if PeriodTrans."Transaction Code" <> 'TXCHRG' then begin  //PKK
                                ...
                                if strGrpText <> PeriodTrans."Group Text" then begin
                                    ...
                                    if PeriodTrans."Employee Classification" <> "PR Salary Card".Currency then  //**changes
                                    ...
                                    if PeriodTrans."Transaction Code" = 'GPAY' then begin
                                        Evaluate(TransAmt[ColumnNo, Index], Format(ROUND(AllowancesLCY, 0.01)));
                                        Evaluate(TransAmtFCY[ColumnNo, Index], Format(ROUND(
                                        CurrExchRate.ExchangeAmtLCYToFCY(CalcDate('CM', SelectedPeriod), Currency, PeriodTrans.Amount - AllowancesLCY, "Currency Factor")
                                        , 0.01)));
                                    end;

                                    if PeriodTrans."Transaction Code" = 'NONTAX' then begin
                                    ...
                                    if PeriodTrans."Transaction Code" = 'NPAY' then begin
                                        Evaluate(TransAmt[ColumnNo, Index], Format(ROUND(NPAYLCY, 0.01)));
                                        Evaluate(TransAmtFCY[ColumnNo, Index], Format(ROUND(
                                        CurrExchRate.ExchangeAmtLCYToFCY(CalcDate('CM', SelectedPeriod), Currency, PeriodTrans.Amount - NPAYLCY, "Currency Factor")
                                        , 0.01)));
                                    end;
                                    ...
                                    if PeriodTrans."Transaction Code" = 'TOT-DED' then begin
                                        Evaluate(TransAmt[ColumnNo, Index], Format(ROUND(PeriodTrans.Amount - DeductionsLCY, 0.01)));
                                        Evaluate(TransAmtFCY[ColumnNo, Index], Format(ROUND(
                                        CurrExchRate.ExchangeAmtLCYToFCY(CalcDate('CM', SelectedPeriod), Currency, DeductionsLCY, "Currency Factor")
                                        , 0.01)));
                                    end;
                    ...
                    *******************************************************************************/
                    ...
                    if HREmpBanks.FindSet then
                        repeat
                            if HREmpBanks.Percentage = 0 then begin
                                ...
                                if HREmpBanks.Currency <> '' then begin
                                    HREmpBanks.AmountLCY := HREmpBanks.Percentage / 100 * (NetSalary - NPAYLCY);
                                    ...
                                    HREmpBanks.AmountLCY := HREmpBanks.Percentage / 100 * (NPAYLCY);
                            end;
    ...
    begin
        ...
        if PeriodTransactions.FindSet then
            repeat
                ...
                if PeriodTransactions."Transaction Code" = 'BPAYFULL' then PeriodTransactions."Employee Classification" := "PR Salary Card".Currency;
