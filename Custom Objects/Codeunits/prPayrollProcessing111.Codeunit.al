...
Codeunit 52185501 prPayrollProcessing111
{
    // ++Note
    ...
    var
        ...
        Cmonth: Integer;
        prSalary: Record 52185637;
        ScaleNotches: Record 52185652;
        "notchNo.": Integer;
    ...
    begin
        ...
        //jAMES
        //check if apprasal date in within same month of year
        //create a tempory Basic pay for process or we can update before close of previous month.
        //Note... When closing period Update pay
        //fncloseperiod refer
        ...
        if curBasicPay > 0 then begin
            ...
            if prEmployeeTransactions.Find('-') then begin
                ...
                repeat
                    ...
                    prTransactionCodes.SetRange(prTransactionCodes."Special Transactions", prTransactionCodes."special transactions"::None);
                    if prTransactionCodes.Find('-') then begin
                        ...
                        if prTransactionCodes.Subledger <> prTransactionCodes.Subledger::" " then begin
                            if prTransactionCodes.Subledger = prTransactionCodes.Subledger::Customer then begin
                                ...
                                Customer.Reset;
                                //Customer.SETRANGE(Customer."Staff No",HrEmployee."No.");
    ...
    begin
        ...
        if TCode = 'NPAY' then begin
            ...
            if HRBankAccounts.Find('-') then begin
                repeat
                    ...
                    //         HRBankAccounts.TESTFIELD(HRBankAccounts."Amount to Transfer (%)");
        ...
        //IF TRANSACTION CODE IS NOT NET PAY
        begin
    ...
    begin
        ...
        /*
        //we can update increments based on the month
            HrEmployee.RESET;
            HrEmployee.SETRANGE(HrEmployee.Status, HrEmployee.Status::Approved);
            HrEmployee.SETFILTER(HrEmployee."Date Of Join", '<>%1', 0D);
            IF HrEmployee.FIND('-') THEN BEGIN
            REPEAT
                 //check date and compare with current date
                  Cmonth:=DATE2DMY(HrEmployee."Date Of Join", 2);
                  IF Cmonth=intNewMonth THEN BEGIN
                      //if appraisal Date is reached, modify the the basic Pay
                      prSalary.RESET;
                      prSalary.SETRANGE(prSalary."Employee Code",HrEmployee."No.");
                      IF prSalary.FIND('-') THEN BEGIN
                          //check the grade notches
                          ScaleNotches.RESET;
                          ScaleNotches.SETRANGE(ScaleNotches."Salary Grade", HrEmployee."Salary Grade");
                          ScaleNotches.SETRANGE(ScaleNotches."Salary Amount",prSalary."Basic Pay");
                          IF ScaleNotches.FIND('-') THEN BEGIN
                              "notchNo.":=ScaleNotches.No;
                              "notchNo.":="notchNo."+1;
                          END;
        ...
        */
        ...
        if ControlInfo."Multiple Payroll" then
            prPayrollPeriods.SetRange(prPayrollPeriods."Closed By", PayrollCode);
