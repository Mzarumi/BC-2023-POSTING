table 52185438 "Loan Guarantors and Security2"
{
    ...
    fields
    {
        ...
        field(2; "Savings Account No./Member No."; Code[20])
        {
            TableRelation = IF ("Guarantor Type" = CONST(Guarantor)) "Savings Accounts"."No." WHERE("Product Category" = FILTER("Deposit Contribution"),
                                                                                                   Balance = FILTER(> 0))
            ELSE
            IF ("Guarantor Type" = CONST(Collateral)) Members."No." WHERE(Status = CONST(Active))
            ELSE
                ...
                if "Guarantor Type" = "Guarantor Type"::Guarantor then begin
                    ...
                    if Cust.Get("Savings Account No./Member No.") then begin
                        ...
                        if Loans.Find('-') then begin
                            ...
                            if Loans."Approved Amount" >= GenSetUp."Self Guarantee %" * "Deposits/Shares" * 0.01 then
                                Error(Text002, Loans."Approved Amount", "Deposits/Shares")
                            else
                                "Amount Guaranteed" := (GenSetUp."Self Guarantee %" * "Deposits/Shares" * 0.01) - Loans."Approved Amount";
                        end;
                    end;
