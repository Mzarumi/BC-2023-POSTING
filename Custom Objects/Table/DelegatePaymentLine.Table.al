table 52186068 "Delegate Payment Line"
{
    ...
    fields
    {
        ...
        field(8; "Tariff Code"; Code[10])
        {
            ...
            TableRelation = "Region Delegates Tariff".Code WHERE("Payment Frequency" = FIELD("Payment Frequency"),
                                                                  "Region Code" = FIELD("Delegate Code"),
                                                                  "Transaction Type" = FIELD("Transaction Type"));

            trigger OnValidate()
            begin
                GeneralSetUp.Get();
                EndDateSalo := CalcDate('-' + Format(GeneralSetUp."Salary Period"), Today);//+FORMAT(ProdFac."No. of Salary Times")
                if "Delegate MNO." <> '' then begin
                    repeat
                        SalProc.Reset;
                ...
                DelegatePaymentLine.SetRange(Posted, true);
                ...
                if DelegatePaymentLine.Find('-') then begin
                    repeat
                        MyCount += 1;
                        Tariffs.Reset;
                        Tariffs.SetRange(Tariffs.Code, Rec."Tariff Code");
    ...
    keys
    {
        key(Key1; "Code", "Entry No", "Delegate Region", "Payment Frequency")
        {
            Clustered = true;
        }
    }
    ...
    begin
        ...
        if DelegateMemberss.Find('-') then begin
            ...
            DelegatePaymentLines.SetRange("Delegate MNO.", "Delegate MNO.");
            //DelegatePaymentLines.SETRANGE(Code,Code);
            //DelegatePaymentLines.SETRANGE(Posted,TRUE);
            ...
            if DelegatesTariff.Find('-') then begin
                repeat
                    ...
                    /* IF DelegatesPayment.GET(Code,"Payment Frequency") THEN BEGIN
                     DelegatesTariff.RESET;
                     DelegatesTariff.SETRANGE(DelegatesTariff.Code,"Tariff Code");
                     DelegatesTariff.SETRANGE("Region Code",DelegateMemberss."Delegate Code");
                     IF DelegatesTariff.FIND('-') THEN
                       BEGIN
                         IF DelegatesTariff.CalculationMethod=DelegatesTariff.CalculationMethod::Milleage THEN
                           BEGIN
                             DimensionValue.RESET;
                             DimensionValue.SETRANGE(Code,"Electoral Zone");
                             IF DimensionValue.FIND('-') THEN
                             BEGIN
                               DelegatePaymentLine."Amount Payable":=DimensionValue."Mileage (KMs)"*DelegatesTariff."Rate Per KM";
                             END;
                             END ELSE
                           DelegatePaymentLine."Amount Payable":=DelegatesTariff.Amount;
                         END;
                         END;*/
                    DelegatePaymentLine."Delegate Name" := DelegateMemberss."Delegate Name";
                    DelegatePaymentLine."Delegate Code" := DelegateMemberss."Delegate Code";
